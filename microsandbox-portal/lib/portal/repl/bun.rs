//! Bun engine implementation for code execution in a sandboxed environment.
//!
//! This module provides a Bun-based code execution engine that:
//! - Runs Bun code in a subprocess with a custom REPL configuration
...
impl Engine for BunEngine {
    async fn initialize(&mut self) -> Result<(), EngineError> {
        ...
        // Start the Bun process manager in a separate task
        tokio::spawn(async move {
                ...
                .spawn()
            {
                Ok(p) => p,
                Err(e) => {
                    eprintln!("Failed to start Bun process: {}", e);
                    return;
                }
            };
            ...
            let mut stdin = match process.stdin.take() {
                Some(s) => s,
                None => {
                    eprintln!("Failed to open Bun stdin");
                    return;
                }
            };
            ...
            let stdout = match process.stdout.take() {
                Some(s) => s,
                None => {
                    eprintln!("Failed to open Bun stdout");
                    return;
                }
            };
            ...
            let stderr = match process.stderr.take() {
                Some(s) => s,
                None => {
                    eprintln!("Failed to open Bun stderr");
                    return;
                }
            };
                ...
                tokio::select! {
                    ...
                    Some(eval_req) = eval_rx.recv() => {
                        ...
                        let result = async {
                            ...
                            // Add console.log statement for EOE marker
                            code_with_marker.push_str(&format!("console.log('{}');\n", eoe_marker));

                            // Write code to Bun process
                            stdin.write_all(code_with_marker.as_bytes()).await.map_err(|e| {
                                EngineError::Evaluation(format!("Failed to send code to Bun: {}", e))
                            })?;

                            // Add newline to execute the code
                            stdin.write_all(b"\n").await.map_err(|e| {
                                EngineError::Evaluation(format!("Failed to send newline to Bun: {}", e))
                            })?;

                            // Flush to ensure code is processed
                            stdin.flush().await.map_err(|e| {
                                EngineError::Evaluation(format!("Failed to flush code to Bun: {}", e))
                            })?;
                    ...
                    _ = stdout_done_rx.recv() => {
                        eprintln!("Bun stdout handler exited");
                        break;
                    }
                    _ = stderr_done_rx.recv() => {
                        eprintln!("Bun stderr handler exited");
                        break;
                    }
                }
            }
    ...
    async fn eval(
        ...
        let eval_tx = self.eval_tx.as_ref().ok_or_else(|| {
            EngineError::Unavailable("Bun engine not initialized".to_string())
        })?;
        ...
        eval_tx
            ...
            .await
            .map_err(|_| EngineError::Unavailable("Bun process channel closed".to_string()))?;

        // Wait for completion
        done_rx
            .await
            .map_err(|_| EngineError::Unavailable("Bun evaluation cancelled".to_string()))?
    }
...
/// Create a new Bun engine instance
pub fn create_engine() -> Result<Box<dyn Engine>, EngineError> {
    Ok(Box::new(BunEngine::new()))
}
